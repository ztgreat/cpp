...
namespace mongols {
    ...
    long openssl::flags =
            SSL_OP_NO_COMPRESSION | SSL_OP_NO_TICKET | SSL_OP_CIPHER_SERVER_PREFERENCE | SSL_OP_SINGLE_ECDH_USE |
            SSL_OP_SINGLE_DH_USE;
    const int openssl::ssl_session_ctx_id = 1;
    bool openssl::enable_verify = false;
    bool openssl::enable_cache = true;

    openssl::openssl(const std::string &crt_file, const std::string &key_file, openssl::version_t v,
        ...
        if (this->ctx) {
            ...
            if (openssl::enable_cache) {
                ...
                SSL_CTX_set_session_id_context(this->ctx, (const unsigned char *) &openssl::ssl_session_ctx_id,
                                               sizeof(openssl::ssl_session_ctx_id));
            } else {
